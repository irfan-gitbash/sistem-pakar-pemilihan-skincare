...
// Fetch diagnosis entries with skin type and symptoms
$stmt = $db->prepare("
...
<body class="bg-gray-100">
    ...
    <main class="max-w-7xl mx-auto p-4 sm:p-6">
        <h1 class="text-2xl font-bold mb-6">Hasil Diagnosis</h1>
            ...
            <table class="min-w-full bg-white rounded-lg shadow overflow-hidden">
                <thead class="bg-gray-200">
                    <tr>
                        ...
                        <th class="py-3 px-4 text-left">Jenis Kulit</th>
                ...
                <tbody>
                    ...
                    <tr>
                        <td colspan="6" class="text-center py-4">Tidak ada data diagnosis.</td>
                    </tr>
                    <?php else: ?>
                    <?php foreach ($diagnoses as $diag): ?>
                    <tr class="border-t hover:bg-gray-50">
                        <td class="py-2 px-4"><?php echo htmlspecialchars($diag['id']); ?></td>
                        <td class="py-2 px-4"><?php echo htmlspecialchars($diag['skin_type_name']); ?></td>
                        <td class="py-2 px-4 max-w-xs truncate" title="<?php echo htmlspecialchars($diag['symptom_names']); ?>">
                            <?php echo htmlspecialchars($diag['symptom_names']); ?>
                        </td>
                        <td class="py-2 px-4"><?php echo number_format($diag['confidence_score'] * 100, 1); ?>%</td>
                        <td class="py-2 px-4"><?php echo date('d M Y H:i', strtotime($diag['created_at'])); ?></td>
                        <td class="py-2 px-4">
                            <div class="flex space-x-2">
                                <a href="diagnosis_view.php?id=<?php echo $diag['id']; ?>" 
                                ...
                                <a href="diagnosis_edit.php?id=<?php echo $diag['id']; ?>" 
                                   class="text-green-600 hover:text-green-800">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                    </svg>
                                </a>
                                <a href="diagnosis_delete.php?id=<?php echo $diag['id']; ?>" 
                                   onclick="return confirm('Apakah Anda yakin ingin menghapus diagnosis ini?');"
                                   class="text-red-600 hover:text-red-800">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                                    </svg>
                                </a>
                                <a href="diagnosis_export.php?id=<?php echo $diag['id']; ?>" 
                                   target="_blank"
        ...
        <div class="md:hidden space-y-4">
            ...
            <div class="bg-white rounded-lg shadow p-4">
                <div class="flex justify-between items-start mb-2">
                    <div>
                        <span class="text-sm text-gray-500">ID: <?php echo htmlspecialchars($diag['id']); ?></span>
                        <h3 class="font-medium"><?php echo htmlspecialchars($diag['skin_type_name']); ?></h3>
                    </div>
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                        <?php echo number_format($diag['confidence_score'] * 100, 1); ?>%
                    </span>
                </div>
                ...
                <div class="text-xs text-gray-500 mb-3">
                    <?php echo date('d M Y H:i', strtotime($diag['created_at'])); ?>
                </div>
        ...
        <!-- Pagination -->
        <div class="mt-4 flex flex-col sm:flex-row justify-between items-center bg-white p-4 rounded-lg shadow">
            <div class="mb-2 sm:mb-0 text-sm text-gray-600">
